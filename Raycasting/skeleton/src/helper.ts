...
export class Settings extends utils.Callbackable{
  ...
  saveImg: () => void = function(){document.dispatchEvent(new CustomEvent('save'))};
 }
...
mirrorMaterial2.mirror = true;
  ...
  sphere.position.set( - 200/300, - 250/300 + 5/300, - 50/300 );
  scene.add(sphere);
  var sphere2 = new THREE.Mesh( sphereGeometry, phongMaterialRed );
  ...
  var box = new THREE.Mesh( boxGeometry, mirrorMaterial2 );
  box.position.set( - 50/300, - 250/300 + 2.5/300, - 150/300 );
  box.rotation.y = 0.25;
  scene.add(box);
  ...
  // right
  var planeright = new THREE.Mesh( planeGeometry, phongMaterialGreen );
...
export function setupLight(scene: THREE.Scene){
  ...
  light2.position.set( - 200/300, 250/300, 100/300 );
  scene.add(light2);
  lights.push(light2);
